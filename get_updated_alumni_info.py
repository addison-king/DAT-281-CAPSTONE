...
def compare_df(new_alumni, old_alumni, id_num):
    ...
    for i in new_alumni:
        if new_alumni.at[0, i] != old_alumni.at[0, i]:
...
def text_new_alumni_p1(alumni):
    ...
    frame_phone_number = [[sg.In(key='phone1', size=(4,1),
                           ...
                           sg.In(key='phone2', size=(4,1),
                                 default_text=number[1],
                                 change_submits=True, do_not_clear=True),
                           sg.T('-', pad=(0,0)),
                           sg.In(key='phone3', size=(5,1),
                                 default_text=number[2],
                                 change_submits=True, do_not_clear=True)]]

    frame_email = [[sg.In(key='email', default_text=alumni.at[0, 'email'])]]

    frame_missing_val = [[sg.T('', key='main_error',
                               text_color='white', size=(43,1))]]

    layout = [[sg.Text('Edit Alumni Information - Page 1')],
...
def text_new_alumni_p2(alumni):
    ...
    if alumni.at[0, 'gender'] == 'Female':
        ...
        frame_gender = [[sg.Radio('Female', 'gender', key='gender_female'),
                         sg.Radio('Male', 'gender', key='gender_male', default=True)]]

    frame_street_ad = [[sg.In(key='street_address', size=(49,1),
                              default_text=alumni.at[0, 'address'])]]

    if alumni.at[0, 'city'] == 'Pittsburgh':
    ...
    if alumni.at[0, 'state'] == 'Pennsylvania':
    ...
    layout = [[sg.T('Edit Alumni Information - Page 2')],
...
#All conditions met, break, move to next page
        ...
        if event == 'zipcode_input' and len(window[event].Get()) >= 1:
            if not re.match('^\d{5}$', window[event].Get()):
                window['error_zipcode'].Update('Please enter a 5 digit zipcode\n(e.g. 15212).')
                window['zipcode_input'].Widget.configure(highlightcolor='purple', highlightthickness=2)
            else:
                window['error_zipcode'].Update('')
                window['zipcode_input'].Widget.configure(highlightcolor='white')
...
def text_new_alumni_p3(alumni):
    ...
    frame_job = [[sg.In(key='job', size=(55,1),
                        default_text=alumni.at[0, 'job'])]]

    frame_health = [[sg.In(key='health', size=(55,1),
                           default_text=alumni.at[0, 'health_info'])]]
...
def text_new_alumni_p4(alumni):
    ...
    frame_emergency_phone = [[sg.In(key='e_phone1', size=(4,1),
                           ...
                           sg.In(key='e_phone2', size=(4,1),
                                 change_submits=True, do_not_clear=True,
                                 default_text=e_phone[1]),
                           sg.T('-', pad=(0,0)),
                           sg.In(key='e_phone3', size=(5,1),
                                 change_submits=True, do_not_clear=True,
                                 default_text=e_phone[2])],
...
def text_new_alumni_p5(alumni):
    ...
    if alumni.at[0, 'performing_arts'] == 'Yes':
        frame_arts = [[sg.Radio('Yes', 'arts', key='arts_yes', default=True),
                        sg.Radio('No', 'arts', key='arts_no')]]
    else:
        frame_arts = [[sg.Radio('Yes', 'arts', key='arts_yes'),
                        sg.Radio('No', 'arts', key='arts_no', default=True)]]
